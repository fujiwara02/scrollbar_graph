...
export default function EXAM_QUESTION() {
  ...
  const BeforeClick = useCallback((g) => {
  ...
  const AnswerClick = useCallback((g) => {
    ...
    if (isFinished) {
      ...
      navigate("/2_exam_answer/" + wkeys[ans] + "/" + url + "/" + list_count[0] + "/" + list_count[1] + "/" + list_count[2]);
    }
...
<div className={styles.backgroundcolor}>
      ...
      <a className={styles.title5}>{wordslist[Object.keys(wordslist)[ans]]}</a><br></br>
...

{
  ...
  <div className='recordElement' >
    ...
    <div className='reButtonParent'>
      ...
      <button onClick={BeforeClick} className={styles.learnContent}>
        前のページへ戻る
      </button>
